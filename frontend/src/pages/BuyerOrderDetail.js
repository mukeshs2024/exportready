...
function BuyerOrderDetail() {
  ...
  const updateStatus = async (action) => {
    ...
    setSuccess("");
    try {
      await API.post(`/orders/${id}/${action}`, null, {
        params: {
          actor_id: buyerId
        }
      });
  ...
  if (loading) {
    return <div style={{ color: "#4a5568" }}>Loading order...</div>;
  }

  if (!detail) {
    return <div style={{ color: "#4a5568" }}>Order not found.</div>;
  }
  ...
  return (
    <div style={{ display: "grid", gridTemplateColumns: "2fr 1fr", gap: "2rem", alignItems: "start" }}>
      <div style={{ background: "white", borderRadius: "14px", padding: "2rem", border: "1px solid #e2e8f0", boxShadow: "0 10px 20px rgba(15, 30, 58, 0.08)" }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
          <div>
            ...
            <p style={{ marginTop: "0.4rem", color: "#64748b" }}>Exporter: {detail.seller?.company_name || detail.seller?.name || ""}</p>
          </div>
              ...
              <div key={offer.id} style={{ background: "#f8fafc", borderRadius: "12px", padding: "1rem", border: "1px solid #e2e8f0" }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                  <div style={{ fontWeight: 700, color: "#0f1e3a" }}>
                    {`${offer.sender_id === buyerId ? "Buyer" : "Seller"} -> $${Number(offer.price || 0).toFixed(2)}`}
                  </div>
      ...
      <div style={{ background: "white", borderRadius: "14px", padding: "1.75rem", border: "1px solid #e2e8f0", boxShadow: "0 10px 20px rgba(15, 30, 58, 0.08)" }}>
        ...
        <p style={{ color: "#64748b" }}>Accept, reject, or counter the latest offer.</p>
