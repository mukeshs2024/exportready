...
function ImporterRegister() {
  ...
  const submit = async () => {
    ...
    try {
      ...
      setError(err.response?.data?.detail || "Failed to register importer");
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div style={{ maxWidth: "720px", margin: "0 auto", background: "white", padding: "2.5rem", borderRadius: "14px", border: "1px solid #e2e8f0", boxShadow: "0 10px 22px rgba(15, 30, 58, 0.08)" }}>
      <h2 style={{ margin: 0, fontSize: "1.6rem", color: "#0f1e3a", fontWeight: 800 }}>Importer Registration</h2>
      <p style={{ color: "#4a5568", marginTop: "0.5rem" }}>
        Register as an international buyer to access the Exporto marketplace.
      </p>
      ...
      >
        {loading ? "Creating Account..." : "Register as Importer"}
      </button>
    </div>
  );
}
